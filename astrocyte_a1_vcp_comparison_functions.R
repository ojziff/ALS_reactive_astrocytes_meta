...
# gene_coordinates <- gtf %>% dplyr::mutate(ensID = ensemblID, gene_coords = paste(.$seqnames, ":", .$start, "-", .$end, sep = "")) %>% dplyr::select(gene_coords, ensID)
...
tx2gene <- readRDS("/camp/home/ziffo/home/genomes/ensembl/Homo_sapiens.GRCh38.99.chr_patch_hapl_scaff.tx2gene.rds") %>% unique # data.frame with 2 columns: ENST ID & ENSG ID
    ...
    if(flip == "yes"){
      if(dashed.line == "NA"){
        ggplot(data2, aes(x = x, y= y, color = x)) + 
          ...
          stat_pvalue_manual(stats_test, label = "p.adj.signif", tip.length = tip.length, x = x.position, hide.ns = TRUE)
      } else{
        ggplot(data2, aes(x = x, y= y, color = x)) + 
          geom_violin() + geom_sina(size = 0.5) + geom_boxplot(data = data2, aes(fill = x), colour = "black", width=width, outlier.shape = NA) +
          scale_colour_manual(values = get_palette("npg",colours)) + scale_fill_manual(values = get_palette("npg",colours)) +  
          ...
          stat_pvalue_manual(stats_test, label = "p.adj.signif", tip.length = tip.length, x = x.position, hide.ns = TRUE) # add manually p values from binomial test
      }
    } else{
      if(dashed.line == "NA"){
        ggplot(data2, aes(x = x, y= y, color = x)) + 
...
# DESeq2 Functions ------------------
curated_profiles <- 
  function(gprofiles = IRevents_gprofiles_curated, colours = 3){
    ...
    if(colours < 4){
      ggplot(gprofiles, aes( x = term_name, y = -log10(p_value), fill = query) ) + 
        ...
        theme(strip.background = element_blank(), legend.position = c(0.8, 0.2), strip.text.y = element_text(size=8, face = "bold", angle = 90), axis.text.y= element_text(size = 8)) +
...
volcano_plot_deseq <- 
  function(data = mn_d25.res, labels_list = volcano.labels){
    ...
    ggplot(data = data2, aes(x = log2FoldChange, y =  -log10(padj), colour = direction)) +  geom_point(size = 0.5) + #xlim(1,5) + ylim(-3,3) +
      scale_colour_manual( values = c(unchanged = "darkgray", upregulated = "firebrick2", downregulated = "dodgerblue3") ) +  theme_bw() +  theme(panel.grid = element_blank(), legend.title = element_blank(), legend.position = "none") + 
...
histogram_plot_irfinder <- 
  function(ctrl = ac_who_ctrl.IR, mutant = ac_who_vcp.IR, ctrl.name = "ctrl", mutant.name = "VCP", legend = TRUE){
    ...
    ggplot(filter(ctrl_and_mutant.filt, IRratio > 0), aes(x = IRratio, fill = reliable)) + 
      ...
      facet_grid(. ~ condition) + scale_fill_manual( values = c("dodgerblue3", "firebrick2") ) + scale_colour_manual( values = c("dodgerblue3", "firebrick2") )
    } else{
    ggplot(filter(ctrl_and_mutant.filt, IRratio > 0), aes(x = IRratio, fill = reliable)) + 
      geom_histogram(position="dodge",binwidth=0.03) +  theme_bw() + 
...
ma_plot_irfinder <- 
  function(data = ac_ctrl_cyt_vs_nuc, xlabs = "Log[10] exon + intron coverage", labels_list = astrocyte.reactivity.labels){
    ...
    ggplot(data = ggmaplot, aes(x = baseMean, y = IRratio.diff, colour = p0.05_reliable_IRdirection)) +  geom_point(size = 0.5) + #xlim(1,5) + ylim(-3,3) +
        scale_colour_manual( values = c("firebrick2", "dodgerblue", "darkgray") ) +  theme_bw() +  
...
track_viewer <- function(gene_test = "SFPQ", txt = ac_who_vcp_vs_ctrl, mut.bam = ac_who_vcp, ctrl.bam = ac_who_ctrl, xpad = 1000){
  ...
  ids <- top_event %>% dplyr::pull(ensID) # get ENSG of gene_test
...
track_viewer_tx_top_IRratio_event <- function(gene_test = "RECK", txt = ac_who_vcp_vs_ctrl, mut.bam = ac_who_vcp, ctrl.bam = ac_who_ctrl, path = NULL){
  ...
  print("Saving figure.")
  png(file = paste("/camp/home/ziffo/home/projects/astrocyte-meta-analysis/splicing/IRFinder/figures/coverage/", path, gene_test, "_topDeltaIRratio_tx_coverage_plot.png", sep = ""), height = 2.2, width = 7, units ="in", res = 300)
  vp <- viewTracks(trackList, gr=gr, viewerStyle=viewerStyle)
  addGuideLine(c(st, en), vp=vp, col = "firebrick2", lwd = 2, lty = "dashed")
  grid.text(IRratio_ctrl, x=.57, y=.65, just="bottom")
  grid.text(IRratio_vcp, x=.57, y=.2, just="bottom")
...
track_viewer_fractions <- function(gene_test = "RND3", txt = ac_vcp_vs_ctrl.filt, mut.nuc.bam = ac_nuc_vcp, ctrl.nuc.bam = ac_nuc_ctrl, mut.cyt.bam = ac_cyt_vcp, ctrl.cyt.bam = ac_cyt_ctrl, path = NULL){
