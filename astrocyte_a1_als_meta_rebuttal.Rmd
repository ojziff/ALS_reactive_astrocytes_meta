...
Run the below DESeq2 objects to create ac_comparison.RData as a single script. Takes \~ 1 hour mins

```{bash}
cd /camp/lab/luscomben/home/users/ziffo/projects/astrocyte-a1-als-meta/scripts
mls
sar
sbatch -N 1 -c 8 --mem=20G -t 4:00:00 --wrap="Rscript /camp/lab/luscomben/home/users/ziffo/projects/astrocyte-a1-als-meta/scripts/astrocyte_a1_als_meta_rebuttal_objects.R" --mail-type=ALL,ARRAY_TASKS --mail-user=oliver.ziff@crick.ac.uk --job-name=DESeq2objects
```
...
patani <- read_csv("/camp/lab/luscomben/home/shared/projects/patani-collab/astrocyte-fractionation-bulk-rnaseq/sample-details/sample_table.csv") %>% filter(fraction == "whole", cellline != "ctrl2") %>% distinct(fraction_cellline, .keep_all = TRUE) %>% # 
  mutate(replicate = c(1,2,1,2), sample = cellline, file_salmon = file.path(dir, paste0("ac_who_", vcp, "_R", replicate), "quant.sf"), dataset = "patani", 
         group = gsub("vcp", "als", vcp), group = factor(group, levels = c("ctrl", "als")), region = "ipsc", celltype_source = "This_study", rpt = 1:4, name = paste("astrocyte_ipsc", rpt, sep = "_")) %>% 
...
tyzack <- read_excel("/camp/lab/luscomben/home/shared/projects/patani-collab/public-data/astrocyte-sod1-ipsc-tyzack-2017/sample-details/metadata.xlsx") %>%
  mutate(replicate = c(1,2,3,4,1,2,3), sample = paste("ac_tyz_", condition, "_R", replicate, sep = ""), file_salmon = file.path(dir, paste0("ac_tyz_", condition, "_R", replicate), "quant.sf"), dataset = "tyzack", 
         group = gsub("sod1", "als", condition), group = factor(group, levels = c("ctrl", "als")), rpt = 1:7, name = paste0("ac_tyz_astrocyte_ipsc_R", rpt), region = "ipsc", celltype_source = "Tyzack_ipsc") %>% 
  select(group, replicate, sample, dataset, file_salmon, celltype_source, region, name)
...
neyrinck <- read_csv("/camp/lab/luscomben/home/shared/projects/patani-collab/public-data/astrocyte-fus-ipsc-neyrinck-2021/sample-details/samplesheet.csv") %>% filter(celltype == "ipsc", overexpressed == "sox9oe", day == 48) %>% # remove primary astrocytes & no TF oe
  mutate(file_salmon = file.path(dir, paste0(group, "_R", replicate), "quant.sf"), sample = paste("ac_ney_", group, "_R", replicate, sep = ""),
         ...
         region = case_when(grepl("ipsc", condition) ~ "ipsc", TRUE ~ "cortex"), celltype_source = paste0("Neyrinck_", region), name = paste0("ac_ney_astrocyte_", region, "_R", rpt)) %>% 
  select(group, replicate, sample, dataset, file_salmon, celltype_source, region, name)
...
barbar <- read_csv("/camp/lab/luscomben/home/shared/projects/patani-collab/public-data/reactive-astrocyte-barbar-2020/sample-details/samplesheet.csv") %>% 
  mutate(file_salmon = file.path(dir, paste(group, "_R", replicate, sep = ""), "quant.sf"), reactivity = c("a0","a1","a0","a1","a0","a1"), group = factor(reactivity, levels = c("a0", "a1")), rpt = c(1,2,3,4,5,6), region = "ipsc", celltype_source = "Barbar_ipsc", name = paste("CD49_astrocyte_ipsc", rpt, sep = "_"), dataset = "barbar") %>% select(group, replicate, sample, dataset, file_salmon, celltype_source, region, name)
...
leng <- read_csv("/camp/lab/luscomben/home/shared/projects/patani-collab/public-data/reactive-astrocyte-ipsc-leng-2021/sample-details/samplesheet.csv") %>% filter(protocol != "Fernandopulle") %>% # remove protocol Fernandopulle
  mutate(sample = paste0(group, "_R", replicate), file_salmon = file.path(dir, sample, "quant.sf"), rpt = c(rep(1:6, each = 4)), region = "ipsc", celltype_source = paste0(protocol,"_",region), name = paste0(protocol,"_ipsc_",rpt), group = factor(treatment, levels = c("a0", "a1"))) %>% select(group, replicate, sample, dataset = protocol, file_salmon, celltype_source, region, name)
...
guttenplan.A1 <- read_csv("/camp/lab/luscomben/home/shared/projects/patani-collab/public-data/reactive-astrocyte-mouse-guttenplan-2020/sample-details/samplesheet.csv") %>% filter(celltype == "astrocyte", mutation == "wt") %>%
  mutate(sample = paste(group, "_R", replicate, sep = ""), file_salmon = file.path(dir, sample, "quant.sf"), reactivity = c("A1","A1","A1","A0","A0","A0"), group = factor(reactivity, levels = c("A0", "A1")), dataset = "guttenplan") %>% 
  select(group, replicate, sample, dataset, file_salmon)
...
guttenplan.sod1 <- read_csv("/camp/lab/luscomben/home/shared/projects/patani-collab/public-data/reactive-astrocyte-mouse-guttenplan-2020/sample-details/samplesheet.csv") %>% filter(celltype == "astrocyte", treatment == "untx") %>%
  mutate(sample = paste(group, "_R", replicate, sep = ""), file_salmon = file.path(dir, sample, "quant.sf"), group = factor(mutation, levels = c("wt", "sod1")), condition = case_when(group == "sod1" ~ "als", group == "wt" ~ "ctrl"), dataset = "guttenplan") %>% 
...
peng <- read_csv("/camp/lab/luscomben/home/shared/projects/patani-collab/public-data/astrocyte-tdp43-mouse-peng-2020/sample-details/samplesheet.csv") %>% filter(group != "mou_tdp43_het") %>%
  mutate(sample = paste(group, "_R", replicate, sep = ""), file_salmon = file.path(dir, paste(group, "_R", replicate, sep = ""), "quant.sf"), condition = gsub("mou_tdp43_", "", group), group = factor(condition, levels = c("ctrl", "ko")), condition = case_when(group == "ko" ~ "als", group == "ctrl" ~ "ctrl"), dataset = "peng") %>% select(group, replicate, sample, file_salmon, condition, dataset)
...
jiang <- read_csv("/camp/lab/luscomben/home/shared/projects/patani-collab/public-data/astrocyte-membralin-mouse-jiang-2019/sample-details/samplesheet.csv") %>% 
  mutate(sample = paste(group, "_R", replicate, sep = ""), file_salmon = file.path(dir, paste(group, "_R", replicate, sep = ""), "quant.sf"), condition = gsub("mou_mem_", "", group), group = factor(condition, levels = c("ctrl", "ko")), condition = case_when(group == "ko" ~ "als", group == "ctrl" ~ "ctrl"), dataset = "jiang") %>% select(group, replicate, sample, file_salmon, condition, dataset)
...
anderson <- read_csv("/camp/lab/luscomben/home/shared/projects/patani-collab/public-data/astrocyte-sci-mouse-anderson-2017/sample-details/samplesheet.csv") %>% 
  mutate(sample = paste(group, "_R", replicate, sep = ""), file_salmon = file.path(dir, paste(group, "_R", replicate, sep = ""), "quant.sf"), group = factor(group, levels = c("ctrl", "sci")), 
         condition = case_when(group == "sci" ~ "als", group == "ctrl" ~ "ctrl"), dataset = "anderson") %>% select(group, replicate, sample, file_salmon, condition, dataset)
...
zhang_bradley_patani_tyzack_birger_neyrinck.pca <- ggplot(zhang_bradley_patani_tyzack_birger_neyrinck.pcaData, aes(PC1, PC2, color=celltype_source, shape = region)) + 
  # scale_fill_manual(values = colorRampPalette(brewer.pal(8, "Paired"))(16)) + 
...
ggsave(zhang_bradley_patani_tyzack_birger_neyrinck.clustering, filename = "/camp/home/ziffo/home/projects/astrocyte-a1-als-meta/expression/deseq2/FigS1.zhang_bradley_patani_tyzack_birger_neyrinck.clustering.png", height = 13, width = 10)
```
...
vsd_counts.celltype.marker <-  assay(ctrls.zhang_bradley_patani.vsd) %>% as_tibble(rownames = "gene_id") %>% left_join(gene2ens, by = "gene_id")  %>% filter(gene_name %in% astrocyte.genes) %>% 
 rowwise() %>%
 mutate("This study" = mean(c_across(starts_with("astrocyte_ipsc")), na.rm = TRUE), 
        # "Barbar et al" = mean(c_across(starts_with("CD49_astrocyte_ipsc")), na.rm = TRUE), 
  ...
  select(gene_name, Myeloid, Neuron, Oligodendrocyte, `Astrocyte adult`, `Astrocyte fetal`, `Bradley et al`, `This study`, -gene_id) %>%
  ...
  transpose_tibble(old_rownames = "group", new_rownames = "gene_name") #%>% arrange(`This study`)# transpose back: columns > rows, order by expression in our astrocytes
...
astrocyte.marker.heatmap <- Heatmap(vsd_counts.celltype.marker.mat, 
          ...
          show_row_names = TRUE, row_names_side = "right", row_names_gp = gpar(fontsize = 7), #row_labels = celltype.marker.row.labels,
          ...
          column_names_rot = 90, 
          column_title = NULL,
          column_names_gp = gpar(col = c("black", "black", "black", "black", "black", "black", "red"), fontsize = c(10,10,10,10,10,10,11)))
astrocyte.marker.heatmap
# 
# 
# # plot heatmap of astrocyte & MN markers
# vsd_counts.marker <- assay(ctrls.zhang_bradley_patani.vsd) %>% as_tibble(rownames = "gene_id") %>% left_join(gene2ens, by = "gene_id")  %>% filter(gene_name %in% astrocyte.genes) %>% rowwise() %>%
...
#   select(gene_name, astrocyte_ipsc_Clarke, astrocyte_ipsc_Barbar, astrocyte_ipsc_Bradley_VSC, astrocyte_fetal, astrocyte_adult, oligodendrocyte, neuron, myeloid, -gene_id) %>%
...
# vsd_counts.marker.mat <- make_matrix(select(vsd_counts.marker,-gene_name), pull(vsd_counts.marker,gene_name))
# 
...
vsd_counts.glutamate.uptake <-  assay(ctrls.zhang_bradley_patani.vsd) %>% as_tibble(rownames = "gene_id") %>% left_join(gene2ens, by = "gene_id") %>% 
 ...
 mutate("This study" = mean(c_across(starts_with("astrocyte_ipsc")), na.rm = TRUE), 
        # "Barbar et al" = mean(c_across(starts_with("CD49_astrocyte_ipsc")), na.rm = TRUE), 
        "Bradley et al" = mean(c_across(starts_with("ventral_spinalcord")), na.rm = TRUE), 
        #dsc_astrocyte_ipsc = mean(c_across(starts_with("dorsal_spinalcord")), na.rm = TRUE), 
        # astrocyte_ipsc_forebrain = mean(c_across(starts_with(c("ventral_forebrain","dorsal_forebrain"))), na.rm = TRUE), #dfb_astrocyte_ipsc = mean(c_across(starts_with("dorsal_forebrain")), na.rm = TRUE), 
        "Astrocyte fetal" = mean(c_across(starts_with("astrocyte_fetal")), na.rm = TRUE), 
        "Astrocyte adult" = mean(c_across(starts_with("astrocyte_adult_temporal")), na.rm = TRUE), 
        Oligodendrocyte = mean(c_across(starts_with("oligodendrocyte")), na.rm = TRUE), 
  ...
  mutate_if(is.numeric, funs((. - 3.309)))  %>% # subtract from all values so start from 0 
  # transpose_tibble(old_rownames = "gene_name", new_rownames = "group") %>% # transpose: rows > columns
  # mutate(across(where(is.numeric), ~ scale2(.x, na.rm = FALSE))) %>% # scale to the columns (i.e. row scale) 
  # transpose_tibble(old_rownames = "group", new_rownames = "gene_name") %>% # transpose back: columns > rows, order by expression in our astrocytes
  arrange(`This study`)
...
glutamate.uptake.heatmap <- 
  Heatmap(vsd_counts.glutamate.uptake.mat, 
          ...
          right_annotation = rowAnnotation(markers = anno_mark(at = glutamate.uptake.row.index, labels = glutamate.uptake.row.labels, labels_gp = gpar(fontsize = 7))),
...
# plot glutamate uptake %
ctrl.glutamate.uptake_assay.stats_test <- ctrl.glutamate.uptake_assay %>% wilcox_test(intensity ~ treatment) %>% add_significance() %>% add_xy_position(x = "treatment")
ctrl.glutamate.uptake_assay.stats_test

ctrl.glutamate.uptake_assay.plot = ggboxplot(ctrl.glutamate.uptake_assay, x="treatment", y = "intensity", add = "point", color = "treatment") + 
  ...
  xlab(expression()) + ylab(expression(Glutamate~uptake~"%")) +
  stat_pvalue_manual(ctrl.glutamate.uptake_assay.stats_test, label = "p.adj.signif", tip.length = 0.01, hide.ns = TRUE, bracket.nudge.y = -10) 
ctrl.glutamate.uptake_assay.plot
...
ctrl.glutamate.uptake_assay.plot = ggplot(ctrl.glutamate.uptake_assay, aes(x=treatment, y = foldchange, color=treatment)) + geom_point() + 
  theme_bw() + theme(legend.position = "none") + scale_colour_manual(values = c("dodgerblue2", "firebrick2", "forestgreen")) + scale_fill_manual(values = c("dodgerblue2", "firebrick2", "forestgreen")) +
...
# ggplot(ctrl.glutamate.uptake_assay, aes(x=treatment, y = intensity)) + #group = treatment,   fill = treatment
#   theme_bw() + theme(legend.title = element_blank()) + scale_colour_manual(values = c("dodgerblue2", "firebrick2", "forestgreen")) + scale_fill_manual(values = c("dodgerblue2", "firebrick2", "forestgreen")) +
#    xlab(expression()) + ylab(expression(Glutamate~uptake~"%")) +
#   geom_point(position=position_dodge(0.8)) +
#   stat_summary(fun.data = mean_se, fun.args = list(mult=1), geom="errorbar", width=0.4, position=position_dodge(0.8)) +  # standard error of mean bars - alternative: stat_summary(fun.data = mean_cl_normal, geom = "errorbar", fun.args = list(mult = 1)) +
#   stat_summary(fun=mean, geom="bar", position=position_dodge(0.8)) + 
#   stat_pvalue_manual(ctrl.glutamate.uptake_assay.stats_test, label = "p.adj.signif", tip.length = 0.01, hide.ns = TRUE, bracket.nudge.y = -20) 


# # Phagocytosis
# ctrl.phagocytosis_assay = read_excel("/camp/home/ziffo/home/projects/astrocyte-a1-als-meta/assay/ctrl_glutamate.uptake_phagocytosis_assay_by_17aug21.xlsx", sheet = "phagocytosis") %>% 
#   mutate(treatment = factor(treatment, levels = c("untreated", "Cytochalasin D")), time = as.factor(hours)) %>%
#   pivot_longer(cols = c(NCRM1.1, VCPF10, C1, C5, NCRM1.2), names_to = "cellline", values_to = "intensity")
# 
# ctrl.phagocytosis_assay.stats_test <- ctrl.phagocytosis_assay %>% group_by(time) %>% t_test(intensity ~ treatment) %>% add_significance() %>% add_xy_position(x = "time")
# ctrl.phagocytosis_assay.stats_test
# 
# ctrl.phagocytosis_assay.plot = ggplot(ctrl.phagocytosis_assay, aes(x=time, y = intensity, group = treatment, color=treatment)) + 
...
#   stat_summary(fun.data = mean_se, fun.args = list(mult=1), geom="errorbar", width=0.4, position=position_dodge(0.8)) +  # standard error of mean bars - alternative: stat_summary(fun.data = mean_cl_normal, geom = "errorbar", fun.args = list(mult = 1)) +
#   stat_summary(fun=mean, geom="crossbar", position=position_dodge(0.8)) + 
#   stat_pvalue_manual(ctrl.phagocytosis_assay.stats_test, label = "p.signif", tip.length = 0.01, hide.ns = TRUE, bracket.nudge.y = -0.3) 
# ctrl.phagocytosis_assay.plot
...
# patani.phagocytosis_fgseaRes <- fgsea(pathways = phagocytosis_genes.list, stats = patani.geneList)
# patani.phagocytosis_fgseaRes
# # pathway      pval      padj    log2err        ES     NES size                         leadingEdge
# # 1: phagocytosis 0.6338028 0.6338028 0.09754492 0.2385496 0.94727  240 SIRPB1,LBP,VAV1,NCF4,FPR2,PPARG,...
# patani.phagocytosis.gseaplot = as.ggplot(plotEnrichment(phagocytosis_genes.list[["phagocytosis"]], patani.geneList)) + labs(subtitle = "VCP Phagocytosis GSEA") + theme(plot.subtitle = element_text(hjust = 0.5))
# patani.phagocytosis.gseaplot
...
mass_spec.volcano <- ggplot(data = mass_spec.res.volcano, aes(x = lfc, y =  -log10(pval), colour = direction)) +  geom_point(size = 0.5) + #xlim(1,5) + ylim(-3,3) + 
  ...
  geom_segment(aes(x = 0.8, xend = 2.8, y= 2.6, yend= 2.6), arrow=arrow(length=unit(0.3,"cm")), color = "firebrick2" ) +
  geom_segment(aes(x = -0.8, xend = -2.8, y= 2.6, yend= 2.6),arrow=arrow(length=unit(0.3,"cm")), color = "dodgerblue2") +
...
# plot standard error bars
vcp.glutamate.uptake_assay.plot = ggplot(glutamate.uptake_assay, aes(x=group, y = foldchange, color=group)) + 
  geom_point() + 
  ...
  stat_summary(fun.data = mean_se, fun.args = list(mult=1), geom="errorbar", width=0.4, position=position_dodge(0.8)) +  # standard error of mean bars - alternative: stat_summary(fun.data = mean_cl_normal, geom = "errorbar", fun.args = list(mult = 1)) +
  stat_summary(fun=mean, geom="crossbar", position=position_dodge(0.8)) + 
  stat_pvalue_manual(glutamate.uptake_assay.stats_test, label = "p.signif", tip.length = 0.01, hide.ns = TRUE, bracket.nudge.y = -0.1) 
vcp.glutamate.uptake_assay.plot
...
Merge data without normalising glutamate uptake %

```{r}
# # Average data
# glutamate.uptake_assay.stats_test <- glutamate.uptake_assay %>% t_test(mean ~ group) %>% add_significance() %>% add_xy_position(x = "group")
# glutamate.uptake_assay.stats_test
...
# # 1 mean  CTRL   VCP        9     8      1.70  15.0 0.109 ns             92.1 <chr [2]>        1     2
# 
# #boxplot
# vcp.glutamate.uptake_assay.boxplot = ggboxplot(glutamate.uptake_assay, x="group", y = "mean", add = "point", color = "group") + 
#   theme_bw() + theme(legend.position = "none") + scale_colour_manual(values = c("dodgerblue2", "firebrick2", "forestgreen")) + scale_fill_manual(values = c("dodgerblue2", "firebrick2", "forestgreen")) +
#   xlab(expression()) + ylab(expression(Glutamate~uptake~"%")) +
#   stat_pvalue_manual(glutamate.uptake_assay.stats_test, label = "p", tip.length = 0.01, hide.ns = FALSE, bracket.nudge.y = -10) 
# vcp.glutamate.uptake_assay.boxplot
# 
# # plot standard error bars
# vcp.glutamate.uptake_assay.plot = ggplot(glutamate.uptake_assay, aes(x=group, y = mean, color=group)) + 
#   geom_point() +
#   theme_bw() + theme(legend.title = element_blank()) + scale_colour_manual(values = c("dodgerblue2", "firebrick2")) +# scale_fill_manual(values = c("dodgerblue2", "firebrick2")) +
#    xlab(label = "") + ylab(expression(Glutamate~uptake~"%")) +
#   stat_summary(fun.data = mean_se, fun.args = list(mult=1), geom="errorbar", width=0.4, position=position_dodge(0.8)) +  # standard error of mean bars - alternative: stat_summary(fun.data = mean_cl_normal, geom = "errorbar", fun.args = list(mult = 1)) +
#   stat_summary(fun=mean, geom="crossbar", position=position_dodge(0.8)) + 
#   stat_pvalue_manual(glutamate.uptake_assay.stats_test, label = "p", tip.length = 0.03, hide.ns = FALSE, bracket.nudge.y = 0) 
# vcp.glutamate.uptake_assay.plot
# 
# # plot technical repeats
# glutamate.uptake_assay.long.stats_test <- glutamate.uptake_assay.long %>% t_test(intensity ~ group) %>% add_significance() %>% add_xy_position(x = "group")
# glutamate.uptake_assay.long.stats_test
# # .y.       group1 group2    n1    n2 statistic    df       p p.signif y.position groups      xmin  xmax
# #   <chr>     <chr>  <chr>  <int> <int>     <dbl> <dbl>   <dbl> <chr>         <dbl> <named li> <dbl> <dbl>
# # 1 intensity CTRL   VCP       27    24      2.78  48.7 0.00768 **             102. <chr [2]>      1     2
# 
# vcp.glutamate.uptake_assay.long.plot = ggplot(glutamate.uptake_assay.long, aes(x=group, y = intensity, color=group)) + 
#   geom_point() +
#   theme_bw() + theme(legend.title = element_blank()) + scale_colour_manual(values = c("dodgerblue2", "firebrick2")) +# scale_fill_manual(values = c("dodgerblue2", "firebrick2")) +
...
#   stat_summary(fun.data = mean_se, fun.args = list(mult=1), geom="errorbar", width=0.4, position=position_dodge(0.8)) +  # standard error of mean bars - alternative: stat_summary(fun.data = mean_cl_normal, geom = "errorbar", fun.args = list(mult = 1)) +
#   stat_summary(fun=mean, geom="crossbar", position=position_dodge(0.8)) + 
#   stat_pvalue_manual(glutamate.uptake_assay.long.stats_test, label = "p", tip.length = 0.03, hide.ns = FALSE, bracket.nudge.y = 0) 
# vcp.glutamate.uptake_assay.long.plot
```
...
write_csv(filter(mass_spec.res, pval < 0.05), "/camp/home/ziffo/home/projects/astrocyte-a1-als-meta/expression/mass-spec/TableS2.mass_spec.csv")
...
als_datasets.dge_numbers <- c(sum(als_datasets.res$padj <= 0.05 & als_datasets.res$log2FoldChange >= 0, na.rm=TRUE), sum(als_datasets.res$padj <= 0.05 & als_datasets.res$log2FoldChange <= 0, na.rm=TRUE))
als_datasets.dge_summary  <- as.data.frame(matrix(data = als_datasets.dge_numbers , nrow = 2, ncol = 1))
colnames(als_datasets.dge_summary) <- "de_genes"
als_datasets.dge_summary$direction <- c("ALS Up","ALS Down")
als_datasets.dge_summary$sample <- "als_datasets"
als_datasets.dge_histogram <- ggbarplot(als_datasets.dge_summary, x="direction" , y="de_genes", fill="direction", label = TRUE, lab.pos = "in", position = position_dodge(0.75), xlab = "") +
...
```{r}
als_datasets.dge_genes <- als_datasets.res %>% filter(padj < 0.05) %>% group_split(log2FoldChange > 0) %>%  map("gene_id") # create list of IR up/down & Expression up/down genes
als_datasets.dge_genes <- gost(als_datasets.dge_genes) # run gprofiler2 on all groups - runs on all KEGG, GO, REAC,TF, WP pathways simultaneously
...
als_datasets.dge_gprofiles_filt_combined$term_name <- gsub("extracellular matrix structural constituent conferring compression resistance", "extracellular matrix", als_datasets.dge_gprofiles_filt_combined$term_name)
als_datasets.dge_gprofiles_filt_combined$term_name <- gsub("enzyme linked receptor protein signaling pathway", "enzyme linked receptor signaling", als_datasets.dge_gprofiles_filt_combined$term_name)
als_datasets.dge_gprofiles_filt_combined$term_name <- gsub(" pathway", "", als_datasets.dge_gprofiles_filt_combined$term_name)
als_datasets.go_curated <- curated_profiles(gprofiles = als_datasets.dge_gprofiles_filt_combined) + theme(legend.position = "none") + labs(subtitle = "pan-ALS")
...
als_datasets_a1.go_curated <- curated_profiles(gprofiles = als.a1.overlap.up_gprofiles_filt.up_down, colours = 2) + labs(subtitle = "A1 & pan-ALS overlap")
als_datasets_a1.go_curated
# ggsave(als_datasets_a1_datasets.go_curated, filename = "/camp/home/ziffo/home/projects/astrocyte-a1-als-meta/expression/deseq2/als_datasets_a1_datasets.go_curated.png", height = 8, width = 5)
```
...
fig3.merged <- ggarrange(
  als_datasets.volcano, als_datasets.go_curated,
  als_datasets.glutamate.uptake.gseaplot,  a1_datasets.A1_ALS_scatter, 
  a1_datasets_als_datasets_venn,als_datasets_a1.go_curated,
  nrow = 3, ncol = 2, labels = c("B", "C", "D", "E", "F", "G"))
# fig3.merged
ggsave(fig3.merged, filename = "/camp/home/ziffo/home/projects/astrocyte-a1-als-meta/expression/deseq2/fig3.merged.png", height = 11, width = 11)
...
guttenplan.sod1.glutamate.gseaplot = plotEnrichment(glutamate.uptake_genes.list[["glutamate.uptake"]], guttenplan.sod1.geneList) + 
  theme(axis.text=element_text(size=8), axis.title=element_text(size=10)) + ylab(expression(Glutamate~Uptake)) + xlab(expression(SOD1)) +  
  annotate("text", x = 100, y = -0.15, label = paste0("NES = ", round(peng.glutamate.uptake_fgseaRes$NES, digits = 3),"\nP = ", round(peng.glutamate.uptake_fgseaRes$pval, digits = 4)), size = 3, hjust = 0)
...
# Fig S9 Mouse ALS & Protective astrocytes
...
# Fig S10 Liddelow genes in mouse models
...
# Fig S11 human & mouse ALS comparison
