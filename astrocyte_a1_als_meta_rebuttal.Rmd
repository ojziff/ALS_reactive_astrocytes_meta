...
mls
sar
sbatch -N 1 -c 8 --mem=30G -t 6:00:00 --wrap="Rscript /camp/lab/luscomben/home/users/ziffo/projects/astrocyte-a1-als-meta/scripts/astrocyte_a1_als_meta_rebuttal_objects.R" --mail-type=ALL,ARRAY_TASKS --mail-user=oliver.ziff@crick.ac.uk --job-name=DESeq2objects
```
...
als_datasets.metadata <- bind_rows(patani, birger, tyzack, neyrinck) %>% mutate(group = factor(group, levels = c("ctrl", "als")))
...
# a1 meta-analysis
a1_datasets.metadata <- bind_rows(barbar, leng) %>% mutate(group = factor(group, levels = c("a0", "a1")))
...
# Zamanian MCAO microarray **Analyse with GEO2R**
...
a1_datasets = DESeq.analysis(metadata = a1_datasets.metadata, design = ~ dataset + group, contrast = "group_a1_vs_a0", species = "human", transcript.level = FALSE)
zhang_bradley_patani_tyzack_birger_neyrinck = DESeq.analysis(metadata = zhang_bradley_patani_tyzack_birger_neyrinck.metadata, design = ~ 1, contrast = "group_als_vs_ctrl", species = "human", transcript.level = FALSE)
ctrls.zhang_bradley_patani = DESeq.analysis(metadata = ctrls.zhang_bradley_patani.metadata, design = ~ 1, contrast = "group_als_vs_ctrl", species = "human", transcript.level = FALSE)
### ADD HERE THE MOUSE ANALYSES
...
astrocyte.marker.heatmap <- Heatmap(vsd_counts.celltype.marker.mat, 
          ...
          column_names_rot = 90, 
          column_title = NULL,
          column_names_gp = gpar(col = c("black", "black", "black", "black", "black", "black", "red"), fontsize = c(9,9,9,9,9,9,10)))
astrocyte.marker.heatmap
