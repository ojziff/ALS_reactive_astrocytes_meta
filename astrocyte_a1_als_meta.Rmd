...
mls
sar
sbatch -N 1 -c 8 --mem=30G -t 6:00:00 --wrap="Rscript /camp/lab/luscomben/home/users/ziffo/projects/astrocyte-a1-als-meta/scripts/ALS_reactive_astrocytes_meta/astrocyte_a1_als_meta_objects.R" --mail-type=ALL,ARRAY_TASKS --mail-user=oliver.ziff@crick.ac.uk --job-name=DESeq2objects
```
...
vsd_counts.celltype.marker <-  ctrls.zhang_bradley_patani$vsd.counts  %>% filter(gene_name %in% astrocyte.genes) %>% rowwise() %>%
 mutate("Current study" = mean(c_across(starts_with("ctrl")), na.rm = TRUE), 
...
glutamate.uptake.heatmap <- 
  Heatmap(vsd_counts.glutamate.uptake.mat, 
          ...
          show_row_names = FALSE, #row_names_side = "right", row_names_gp = gpar(fontsize = 8, fontface = "italic"),
          border = TRUE, #, #, col = c(rep("red", 10), rep("blue", 8)))
          right_annotation = rowAnnotation(markers = anno_mark(at = glutamate.uptake.row.index, labels = glutamate.uptake.row.labels, labels_gp = gpar(fontsize = 8, fontface = "italic"))),
...
patani.volcano <- volcano_plot_deseq(data = patani$res, labels_list = interesting.gene.labels.filt) + xlab(expression(RNA~sequencing~log[2]~fold~change~VCP:CTRL)) + 
...
patani$res %>% filter(padj < 0.05, log2FoldChange > 0) # 91
patani$res %>% filter(padj < 0.05, log2FoldChange < 0) # 108
...
mass_spec.res.volcano$direction %>% table
...
als_datasets$res %>% filter(padj<0.05, log2FoldChange > 0) # 1309
als_datasets$res %>% filter(padj<0.05, log2FoldChange < 0) # 1562
...
a1_datasets.A1_ALS_scatter <- ggscatter(als_datasets_a1_datasets.dge, x = "log2FC.ALS", y = "log2FC.A1", color = "overlap_direction", palette = c("firebrick2", "dodgerblue2", "grey", "grey", "grey", "grey", "grey"), 
    ...
    geom_text_repel(data = filter(als_datasets_a1_datasets.dge, overlap_direction %in%  c("overlapping up", "overlapping down"), gene_name %in% all.reactivity.go, log2FC.A1 < 9), aes(x = log2FC.ALS, y = log2FC.A1, label = gene_name), 
...
a1_datasets_als_datasets_venn_up <- ggvenn(als_datasets.a1_datasets.A1.up_genes, fill_color = c("dodgerblue3", "firebrick1", "forestgreen"), show_percentage = FALSE, stroke_size = 0.4, set_name_size = 3.7) + labs(subtitle = "Overlap Up")  + theme(plot.subtitle = element_text(hjust = 0.5), plot.margin = unit(c(0, -1, -1, -1), "lines"))
a1_datasets_als_datasets_venn_down <- ggvenn(als_datasets.a1_datasets.A1.down_genes, fill_color = c("dodgerblue3", "firebrick1", "forestgreen"), show_percentage = FALSE, stroke_size = 0.4, set_name_size = 3.7) + labs(subtitle = "Overlap Down") + theme(plot.subtitle = element_text(hjust = 0.5), plot.margin = unit(c(0, -1, -1, -1), "lines"))
...
ggsave(a1_datasets.clustering, filename = "/camp/home/ziffo/home/projects/astrocyte-a1-als-meta/expression/deseq2/FigS4.a1_datasets.clustering.png", height = 13, width = 10)
```
...
#             pathway   pval   padj   log2err        ES      NES size                               leadingEdge
# 1: glutamate.uptake 0.0784 0.0784 0.2020717 0.3070159 1.194868  389 PRKAR2B,GRM3,NPTX2,PCDH17,PLPPR4,NRGN,...

ferraiuolo.res %>% filter(pvalue < 0.05, log2FoldChange > 0) # 45
ferraiuolo.res %>% filter(pvalue < 0.05, log2FoldChange < 0) # 29
```
...
3A biorender schematic of MCAO & SCI models
...
# listA size=1309
# listB size=1460
# Intersection size=108
# Overlapping p-value=3.8e-19
# Jaccard Index=0.0
...
# listA size=1309
# listB size=1312
# Intersection size=134
# Overlapping p-value=2e-37
# Jaccard Index=0.1
...
# listA size=1562
# listB size=1460
# Intersection size=171
# Overlapping p-value=1.7e-45
# Jaccard Index=0.1
...
# listA size=1309
# listB size=2622
# Intersection size=215
# Overlapping p-value=9e-65
# Jaccard Index=0.1
...
# listA size=1562
# listB size=2274
# Intersection size=176
# Overlapping p-value=1.2e-38
# Jaccard Index=0.0
...
# listA size=1562
# listB size=2622
# Intersection size=164
# Overlapping p-value=1.6e-25
# Jaccard Index=0.0
```
...
liddelow.astrocyte.reactivity.marker.heatmap <- Heatmap(liddelow.astrocyte.reactivity.marker.mat.scale,
          ...
          row_names_gp = gpar(fontsize = 6, fontface = "italic"), column_names_gp = gpar(fontsize = 8), border = TRUE, #, #, col = c(rep("red", 10), rep("blue", 8)))
...
main.escartin.marker.heatmap <- Heatmap(main.escartin.marker.mat.scale, 
          ...
          row_names_gp = gpar(fontsize = 7, fontface = "italic"), column_names_gp = gpar(fontsize = 8),
...
#   xlab("") + ylab(expression(Normalized~enrichment))
...
# fig3.merged
ggsave(fig3.merged, filename = "/camp/home/ziffo/home/projects/astrocyte-a1-als-meta/expression/deseq2/fig3.merged.pathways.markers.png", height = 12, width = 10)
...
# figS6.merged
ggsave(figS6.merged, filename = "/camp/home/ziffo/home/projects/astrocyte-a1-als-meta/expression/deseq2/figS6.merged.reactive.genes.png", height = 15, width = 11)
...
# progeny.heatmaps
ggsave(progeny.heatmaps, filename = "/camp/home/ziffo/home/projects/astrocyte-a1-als-meta/expression/deseq2/FigS7.progeny.heatmaps.png", height = 15, width = 13.5)
...
# Table S6. Overlap mouse ALS datasets
...
labels_plot <- mouse_als_datasets$res %>% filter(gene_name %in% c(mouse_als_datasets.interesting.gene.labels, mouse_als_datasets.reactive.labels), padj < 0.05)
data2 <- mouse_als_datasets$res %>% mutate(direction = case_when(padj < 0.05 & log2FoldChange > 0 ~ "upregulated", padj < 0.05 & log2FoldChange < 0 ~ "downregulated", TRUE ~ "unchanged"))
mouse_als_datasets.volcano = ggplot(data = data2, aes(x = log2FoldChange, y =  -log10(padj), colour = direction)) +  geom_point(size = 0.5) + #xlim(1,5) + ylim(-3,3) +
  ...
  ylab(expression( -log[10]~FDR )) + xlab(expression(log[2]~fold~change~mouse~ALS:CTRL)) +
  geom_hline(yintercept = -log10(0.05), linetype = 2) + geom_vline(xintercept =0, linetype = 2) +
  geom_text_repel(data = labels_plot, aes(x = log2FoldChange, y = -log10(padj), label = gene_name.mouse), fontface = "italic", colour = "black", min.segment.length = 0, box.padding = 0.5, point.padding = 0.5, size=2.6, max.overlaps = Inf) + 
  coord_cartesian(xlim = c(-3,3), ylim = c(0,8)) +
  geom_segment(aes(x = 1, xend = 3, y= 7, yend= 7), arrow=arrow(length=unit(0.3,"cm")), color = "firebrick2" ) +  geom_segment(aes(x = -1, xend = -3, y= 7, yend= 7),arrow=arrow(length=unit(0.3,"cm")), color = "dodgerblue2") +
...
mouse_als_datasets$res %>% filter(padj < 0.05, log2FoldChange > 0) # 429
mouse_als_datasets$res %>% filter(padj < 0.05, log2FoldChange < 0) # 624
...
```{r}
mouse_als_datasets.dge <- mouse_als_datasets$res %>% select(gene_name, gene_name.mouse, gene_id, log2FC.ALS = log2FoldChange, FDR.ALS = padj)
guttenplan.A1.dge <- guttenplan.A1$res %>% select(gene_name, gene_name.mouse, log2FC.guttenplan.A1 = log2FoldChange, FDR.guttenplan.A1 = padj)
mouse_als_datasets_guttenplan.A1.dge <- full_join(mouse_als_datasets.dge, guttenplan.A1.dge, by = c("gene_name", "gene_name.mouse"))
...
# fig4.merged
ggsave(fig4.merged, filename = "/camp/home/ziffo/home/projects/astrocyte-a1-als-meta/expression/deseq2/fig4.merged.png", height = 12, width = 11)
...
```{r}
mouse_als_datasets_als_datasets.dge %>% filter(overlap_direction %in% c("overlapping up", "overlapping down")) %>% select(gene_name, overlap_direction, log2FC.mouse_ALS, FDR.mouse_ALS, log2FC.human_ALS, FDR.human_ALS) %>%
...
# Table S8. Mouse ALS & A1 gene expression
...
write_csv(als_guttenplan.A1_merged.dge, "/camp/home/ziffo/home/projects/astrocyte-a1-als-meta/expression/deseq2/TableS8.mouse_overlap_als_a1.csv")
```
...
# listA size=429
# listB size=2622
# Intersection size=149
# Overlapping p-value=1.7e-86
# Jaccard Index=0.1
ALS.DGE.down.genes <- mouse_als_datasets$res %>% filter(padj < 0.05, log2FoldChange < 0) %>% pull(gene_name)
SCI.DGE.down.genes <- anderson$res %>% filter(padj < 0.05, log2FoldChange < 0) %>% pull(gene_name)
newGeneOverlap(ALS.DGE.down.genes,SCI.DGE.down.genes, genome.size=genome.size) %>% testGeneOverlap()
...
# Overlapping p-value=2.4e-07
# Jaccard Index=0.0
newGeneOverlap(ALS.DGE.down.genes, SCI.DGE.up.genes, genome.size=genome.size) %>% testGeneOverlap()
# GeneOverlap object:
# listA size=624
# listB size=2622
# Intersection size=166
# Overlapping p-value=2.6e-76
# Jaccard Index=0.1
```
...
# Table S9 Mouse panALS & protective datasets
...
write_csv(mouse_als_protective_datasets_merged.dge, "/camp/home/ziffo/home/projects/astrocyte-a1-als-meta/expression/deseq2/TableS9.overlap_mouse_als_protective_datasets.csv")
...
figS11.merged
ggsave(figS11.merged, filename = "/camp/home/ziffo/home/projects/astrocyte-a1-als-meta/expression/deseq2/figS11.merged.mouse.reactive.genes.png", height = 10, width = 10)
...
## SCI & Sod1.TRAP scatter correlation
...
guttenplan.A1_a1_datasets_cleveland_venn = ggarrange(cleveland.a1_datasets.guttenplan.A1_up_genes_venn, cleveland.a1_datasets.guttenplan.A1_down_genes_venn, ncol = 2)
...
glm1 <- glm(Freq ~ List1 * List2 * List3, family='poisson',data=ftable)
glm2 <- glm(Freq ~ List1*List2 + List1*List3 + List2*List3, family='poisson',data=ftable)
anova(glm1,glm2,test='Chisq')[['Pr(>Chi)']][2] 
# [1] 4.715679e-11
...
figS12.merged
ggsave(figS12.merged, filename = "/camp/home/ziffo/home/projects/astrocyte-a1-als-meta/expression/deseq2/figS12.Sod1.trap.png", height = 10, width = 9)
```
...
write_csv(Sod1.TRAP_guttenplan.A1_merged.dge, "/camp/home/ziffo/home/projects/astrocyte-a1-als-meta/expression/deseq2/TableS10.overlap_Sod1.TRAP_A1.csv")
